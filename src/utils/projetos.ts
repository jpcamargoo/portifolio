...
const projetosRaw: Projeto[] = [
  ...
  },
  {
    id: 'jla-metalworks',
    titulo: {
      pt: 'JLA Metal Works — ERP de Caldeiraria Industrial',
      en: 'JLA Metal Works — Industrial Boilermaking ERP',
    },
    descricao: {
      pt: 'Sistema interno de gestão comercial e operacional para caldeiraria industrial: clientes, propostas técnicas, leads do site público e auditoria — com PDFs gerados sob demanda, autenticação multi-fator de bloqueio e deploy serverless.',
      en: 'Internal commercial and operational management system for industrial boilermaking: customers, technical proposals, public-site leads and auditing — with on-demand PDFs, multi-factor lockout authentication and serverless deployment.',
    },
    contexto: {
      pt: 'Caldeiraria sem CRM: propostas em planilha, leads do site em e-mail solto, histórico de relacionamento em cabeças. O projeto centraliza o ciclo comercial (lead → cliente → proposta → PDF assinado) com trilha de auditoria LGPD-ready e site institucional integrado ao mesmo backend.',
      en: 'Boilermaker without a CRM: proposals in spreadsheets, site leads scattered in email, relationship history in people\'s heads. The project centralizes the commercial cycle (lead → customer → proposal → signed PDF) with an LGPD-ready audit trail and an institutional site integrated to the same backend.',
    },
    stack: ['Next.js 14', 'TypeScript', 'NextAuth v5', 'Prisma 5', 'PostgreSQL (Neon)', 'Tailwind CSS', 'React-PDF', 'AWS S3', 'Upstash Redis', 'Zod', 'Vitest', 'GitHub Actions', 'Vercel'],
    processo: [
      { pt: 'App Router + RSC: páginas server-rendered com Client Components isolados em ilhas — sem hidratação desnecessária', en: 'App Router + RSC: server-rendered pages with Client Components isolated into islands — no unnecessary hydration' },
      { pt: 'NextAuth v5 (Credentials + JWT HS256): lockout progressivo após 5 tentativas, auditoria de LOGIN/LOGIN_FALHA, rotação de secret preparada', en: 'NextAuth v5 (Credentials + JWT HS256): progressive lockout after 5 attempts, LOGIN/LOGIN_FAILURE audit, secret rotation prepared' },
      { pt: 'Pipeline de API tipado (compose): middleware encadeado — auth → role → rate-limit → CSRF → Zod → handler — com erro padronizado em todas as rotas', en: 'Typed API pipeline (compose): chained middleware — auth → role → rate-limit → CSRF → Zod → handler — with standardized error across all routes' },
      { pt: 'Uploads seguros: presigned PUT no S3 (5 min), validação de magic bytes server-side, rate-limit por IP', en: 'Secure uploads: presigned S3 PUT (5 min), server-side magic-byte validation, per-IP rate limit' },
      { pt: 'PDFs server-side: templates React-PDF (proposta comercial + técnica) gerados sob demanda, sem dependência de browser', en: 'Server-side PDFs: React-PDF templates (commercial + technical proposal) generated on demand, no browser dependency' },
      { pt: 'Observabilidade: pino estruturado com transport Logtail opt-in, audit log fail-safe (nunca derruba request)', en: 'Observability: structured pino with opt-in Logtail transport, fail-safe audit log (never breaks the request)' },
      { pt: 'CI/CD: GitHub Actions (lint + typecheck + 61 testes vitest + coverage) → Vercel região gru1', en: 'CI/CD: GitHub Actions (lint + typecheck + 61 vitest tests + coverage) → Vercel region gru1' },
    ],
    impacto: {
      pt: 'Substituição completa de planilha + e-mail por sistema com auditoria. Backend serverless escala sob demanda (Neon + Vercel + S3) sem servidor para gerir. Trilha LGPD desde o dia 1: cada ação sensível registrada com IP, user-agent e diff.',
      en: 'Complete replacement of spreadsheet + email by an audited system. Serverless backend scales on demand (Neon + Vercel + S3) with no server to manage. LGPD trail from day 1: every sensitive action logged with IP, user-agent and diff.',
    },
    categoria: 'fullstack',
    dominio: {
      pt: 'Caldeiraria industrial vende projeto, não commodity: cada proposta é técnica, customizada e tem ciclo longo. O domínio exige rastreabilidade fim-a-fim (quem viu o lead, quem criou a proposta, qual versão foi enviada), separação clara entre lead frio (site público) e cliente ativo (CRM), e geração de documento profissional como artefato final do funil.',
      en: 'Industrial boilermaking sells engineering, not commodity: each proposal is technical, customized and has a long cycle. The domain demands end-to-end traceability (who saw the lead, who created the proposal, which version was sent), clear separation between cold lead (public site) and active customer (CRM), and professional document generation as the final funnel artifact.',
    },
    requisitos: {
      funcionais: [
        { pt: 'RF01 — Captura de leads do site público com rate-limit anti-spam e notificação ao admin', en: 'RF01 — Public-site lead capture with anti-spam rate limit and admin notification' },
        { pt: 'RF02 — CRUD de clientes (PJ/PF) com validação de CNPJ algorítmica', en: 'RF02 — Customer CRUD (legal/individual) with algorithmic CNPJ validation' },
        { pt: 'RF03 — Editor de propostas com versionamento, status (rascunho → enviada → aprovada/rejeitada) e dois templates de PDF', en: 'RF03 — Proposal editor with versioning, status (draft → sent → approved/rejected) and two PDF templates' },
        { pt: 'RF04 — Upload de anexos técnicos com validação de tipo real (magic bytes), não só extensão', en: 'RF04 — Technical-attachment upload with real type validation (magic bytes), not extension alone' },
        { pt: 'RF05 — Auditoria consultável de toda ação CRUD e autenticação', en: 'RF05 — Queryable audit log of every CRUD and authentication action' },
        { pt: 'RF06 — Autenticação com lockout progressivo e roles (ADMIN, COMERCIAL, VISUALIZADOR)', en: 'RF06 — Authentication with progressive lockout and roles (ADMIN, SALES, VIEWER)' },
      ],
      naoFuncionais: [
        { pt: 'RNF01 — Stateless serverless: cada request independente, escala horizontal sem sticky session', en: 'RNF01 — Stateless serverless: each request independent, horizontal scaling without sticky sessions' },
        { pt: 'RNF02 — Cobertura ≥ 85% nas camadas de segurança e validação', en: 'RNF02 — Coverage ≥ 85% on security and validation layers' },
        { pt: 'RNF03 — Resposta de API < 300ms p95 em endpoints sem PDF/upload', en: 'RNF03 — API response < 300ms p95 on non-PDF/upload endpoints' },
        { pt: 'RNF04 — CSP estrita sem unsafe-inline, nonce por request, headers A+ no securityheaders.com', en: 'RNF04 — Strict CSP without unsafe-inline, per-request nonce, A+ headers on securityheaders.com' },
        { pt: 'RNF05 — Zero secrets no bundle client (server-only enforced)', en: 'RNF05 — Zero secrets in the client bundle (server-only enforced)' },
      ],
    },
    decisoesTecnicas: [
      { pt: 'Prisma + PgBouncer (Neon pooler): ORM type-safe sem prepared statements (compatível com transaction-mode); trade-off — algumas queries N+1 precisam de include explícito', en: 'Prisma + PgBouncer (Neon pooler): type-safe ORM without prepared statements (transaction-mode compatible); trade-off — some N+1 queries require explicit include' },
      { pt: 'NextAuth v5 com Credentials: controle total sobre fluxo de authorize (lockout, audit, role); trade-off — sem OAuth pronto, mas necessário para auditoria interna', en: 'NextAuth v5 with Credentials: full control over authorize flow (lockout, audit, role); trade-off — no out-of-the-box OAuth, but required for internal auditing' },
      { pt: 'Pipeline compose em vez de decorators: middleware composável tipado por inferência; trade-off — verbosidade na assinatura, ganho em explicitude', en: 'Compose pipeline instead of decorators: composable middleware typed via inference; trade-off — signature verbosity, gain in explicitness' },
      { pt: 'React-PDF sobre Puppeteer/Chromium: PDF gerado em pure-JS, deploy em serverless sem binário pesado; trade-off — sem CSS completo, layout via primitivas', en: 'React-PDF over Puppeteer/Chromium: PDF generated in pure JS, serverless deploy without heavy binary; trade-off — no full CSS, layout via primitives' },
      { pt: 'Upstash Redis para rate-limit: HTTP-based, funciona em edge sem connection pool; fallback in-memory em dev', en: 'Upstash Redis for rate-limit: HTTP-based, works on edge without connection pool; in-memory fallback in dev' },
      { pt: 'Audit log fail-safe: falha de auditoria nunca derruba operação principal — logada, mas não propagada', en: 'Fail-safe audit log: audit failure never breaks the main operation — logged but not propagated' },
      { pt: 'Zod em duas camadas: validação de env (build-time) + payload (request-time); trade-off — bundle maior, ganho em DX e segurança', en: 'Zod in two layers: env validation (build-time) + payload validation (request-time); trade-off — larger bundle, gain in DX and security' },
    ],
    ipo: {
      input: {
        pt: 'Lead público (form do site) ou ação interna (sessão autenticada + payload Zod-validado)',
        en: 'Public lead (site form) or internal action (authenticated session + Zod-validated payload)',
      },
      process: {
        pt: 'middleware (CSP + guard) → compose pipeline (auth → role → rate-limit → CSRF → validation) → handler com Prisma em transação → audit log assíncrono → response tipado',
        en: 'middleware (CSP + guard) → compose pipeline (auth → role → rate-limit → CSRF → validation) → handler with Prisma transaction → async audit log → typed response',
      },
      output: {
        pt: 'JSON estruturado para CRUD, PDF binário em stream para propostas, presigned URL para uploads e e-mail SMTP para notificações',
        en: 'Structured JSON for CRUD, binary PDF stream for proposals, presigned URL for uploads and SMTP email for notifications',
      },
    },
    dadosAnalise: [
      { pt: 'Schema relacional normalizado: 11 entidades, 9 enums tipados (Role, ClienteStatus, PropostaStatus, LeadStatus, AuditAcao, etc.)', en: 'Normalized relational schema: 11 entities, 9 typed enums (Role, ClienteStatus, PropostaStatus, LeadStatus, AuditAcao, etc.)' },
      { pt: 'Soft delete + auditoria: nenhum DELETE físico em entidades comerciais — deletedAt + AuditLog preservam histórico', en: 'Soft delete + audit: no physical DELETE on commercial entities — deletedAt + AuditLog preserve history' },
      { pt: 'Lead → Cliente como conversão explícita: lead vira cliente via mutation auditada, mantendo a origem rastreável', en: 'Lead → Customer as explicit conversion: lead becomes customer via audited mutation, preserving source traceability' },
      { pt: 'Validação em fronteiras: Zod no boundary HTTP, Prisma no boundary DB — sem validação espalhada', en: 'Boundary validation: Zod at the HTTP boundary, Prisma at the DB boundary — no scattered validation' },
    ],
    metricas: [
      { pt: '61 testes vitest, 88% de cobertura de statements, 100% das funções', en: '61 vitest tests, 88% statement coverage, 100% of functions' },
      { pt: 'Pipeline CI bloqueante: lint + typecheck + testes + coverage em < 90s', en: 'Blocking CI pipeline: lint + typecheck + tests + coverage in < 90s' },
      { pt: 'CSP estrita com nonce dinâmico — zero unsafe-inline em produção', en: 'Strict CSP with dynamic nonce — zero unsafe-inline in production' },
      { pt: 'Lockout após 5 tentativas + audit de cada falha — defesa contra brute-force documentada', en: 'Lockout after 5 attempts + audit of each failure — documented brute-force defense' },
      { pt: 'Deploy serverless multi-região-ready (gru1 default), zero servidor para manter', en: 'Multi-region-ready serverless deploy (gru1 default), zero server to maintain' },
    ],
  },
  {
...
// Ordem de apresentação na página de projetos.
// IDs não listados aparecem ao final, mantendo a ordem original.
const ORDEM_PROJETOS: string[] = [
  'europa-viagens',
  'tourpilot-dashboard',
  'jla-metalworks',
  'gerador-relatorios',
  'automacao-ai-workflow',
  'integracao-dados-sql',
  'api-modular-nodejs',
];

export const projetos: Projeto[] = [
  ...ORDEM_PROJETOS
    .map((id) => projetosRaw.find((p) => p.id === id))
    .filter((p): p is Projeto => Boolean(p)),
  ...projetosRaw.filter((p) => !ORDEM_PROJETOS.includes(p.id)),
];
